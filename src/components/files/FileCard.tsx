...
export function FileCard({ file, onDelete, onShare, onPreview, onRename, onMove, selected, onSelect, selectionMode }: FileCardProps) {
  const { user } = useAuth();
          ...
          <>
            ...
            <div className="absolute right-0 top-8 z-20 bg-gray-800 border border-gray-700 rounded-lg shadow-xl py-1 w-44">
              <button
                ...
                className="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-gray-300 hover:bg-gray-700 cursor-pointer"
              >
              ...
              {canManage && (
                <>
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); onMove?.(); setMenuOpen(false); }}
                    className="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-gray-300 hover:bg-gray-700 cursor-pointer"
                  >
